...
export function reviewSections(result: ReviewResult): ReportSpec['sections'] {
  ...
  sections.push({
    ...
    kind: 'table', description: result.dateKey + ' 按供应商执行数据判定，仅符合项进入汇报。',
    data: result.reportable.slice(0, 30).map(toRow),
  });
  sections.push({
    id: 'review_basic', eyebrow: 'BASELINE', title: '达基础线笔记（正向30条以上）',
    kind: 'table', description: '符合30条基础要求，可正常结算与沉淀。',
    data: result.basic.slice(0, 30).map(toRow),
  });
  return sections;
}
