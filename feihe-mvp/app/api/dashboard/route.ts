...
export async function GET(request: Request) {
  ...
  const project=projectId(params.get('projectId')); const clauses: string[] = ['pn.project_id=?'];
  const values: string[] = [project];
  const from = params.get('from'); const to = params.get('to'); const source = params.get('source');
  const status = params.get('status'); const scope = params.get('scope');
  if (from) { clauses.push('(date(COALESCE(pn.last_fetched_at,n.published_at))>=date(?) OR COALESCE(pn.last_fetched_at,n.published_at) IS NULL)'); values.push(from); }
  if (to) { clauses.push('(date(COALESCE(pn.last_fetched_at,n.published_at))<=date(?) OR COALESCE(pn.last_fetched_at,n.published_at) IS NULL)'); values.push(to); }
  ...
  const [pipelines, noteAgg, supplierAgg, actionAgg, keyComments, notes, trend, sourceDist, scopeDist,
    statusDist, topics, brands, formats, levels, categories, locations, dataQuality, topNotes, adsTotals, adsAccounts] = await Promise.all([
  ...
  const coverIds = topNoteRows.filter((row) => !String(row.coverUrl || '').startsWith('/api/note-covers')).map((row) => String(row.id)).slice(0, 8);
  let cachedCovers: Awaited<ReturnType<typeof cacheNoteCovers>> = [];
  try { cachedCovers = await cacheNoteCovers(coverIds, project, 8); } catch { /* ranking remains usable without a cover */ }
  const coverMap = new Map(cachedCovers.map((item) => [item.noteId, item.coverUrl]));
  for (const row of topNoteRows) if (coverMap.has(String(row.id))) row.coverUrl = coverMap.get(String(row.id)) || '';
  for (const row of resultRows(notes)) if (coverMap.has(String(row.id))) row.coverUrl = coverMap.get(String(row.id)) || '';

  const note = noteAgg || {};
  const positive = Number(note.positiveCount || 0); const negative = Number(note.negativeCount || 0);
  const question = Number(note.questionCount || 0); const commentTotal = Number(note.commentTotal || 0);
  const base = Math.max(1, commentTotal); const readCount = Number(note.readCount || 0);
  const interactionCount = Number(note.interactionCount || 0); const cost = Number(note.creatorCost || 0);
  const exposure = Number(note.exposure || 0);
  return Response.json({
    ok: true, pipelines: pipelines.results,
    metrics: {
      ...note, noteCount: Number(note.noteCount || 0), commentTotal,
      positiveCount: positive, positiveRate: positive / base, negativeCount: negative, negativeRate: negative / base,
      questionCount: question, questionRate: question / base, neutralCount: Math.max(0, commentTotal - positive - negative - question),
      engagementRate: readCount ? interactionCount / readCount : 0,
      cpm: exposure ? cost * 1000 / exposure : 0, cpr: readCount ? cost / readCount : 0,
      cpe: interactionCount ? cost / interactionCount : 0, supplier: supplierAgg || {}, actions: actionAgg || {},
    },
    analytics: {
      trend: resultRows(trend), sourceDistribution: resultRows(sourceDist), scopeDistribution: resultRows(scopeDist),
      statusDistribution: resultRows(statusDist), topics: resultRows(topics), brands: resultRows(brands),
      formats: resultRows(formats), creatorLevels: resultRows(levels), categories: resultRows(categories),
      locations: resultRows(locations), dataQuality: dataQuality || {}, topNotes: topNoteRows,
    },
    keyComments: keyComments.results, notes: notes.results,
    ads: { totals: adsTotals || {}, accounts: adsAccounts.results || [] },
    projectId: project, filters: { from, to, source, status, scope }, syncedAt: new Date().toISOString(),
  });
}
