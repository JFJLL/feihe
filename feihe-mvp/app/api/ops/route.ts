...
export async function GET(request: Request) {
  ...
  await ensureSchema(); const d1 = db(); const url = new URL(request.url);
  const project=projectId(url.searchParams.get('projectId')); const visibility = url.searchParams.get('visibility') || '';
  ...
  const [jobs, logs, supplier, supplierFeatures, categories, rules, acceptance, goals, growth, reviewRules, reports] = await Promise.all([
    ...
    getSetting('acceptance',{reportCount:200,baseCount:30,brandTopRate:.4,freshnessHours:24,supplierSimilarity:.58},project),
    getSetting('goals',{workTarget:0,workCompleted:0,publishTarget:0,budgetTarget:0,commentTarget:0},project),
    getSetting('growth',{watchKeywords:[],inspirations:[],seedNoteIds:[],thresholds:{breakoutInteractions:1000,seedScore:65,ctr:.15,cpuv:.7}},project),
  ...
  return Response.json({ ok: true, jobs: jobs.results, logs: logs.results, supplier: supplier.results, supplierFeatures: supplierFeatures.results, categories: categories.results, reviewRules:reviewRules.results, reports:reports.results, settings: { rules, acceptance, goals, growth } });
}
